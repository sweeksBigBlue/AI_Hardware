...
def train_nerf():
    ...
    # Create synthetic rays (not from camera, but for learning)
    H, W = 32, 32
    rays_o = torch.zeros(H * W, 3).to(device)              # All rays start at origin
    rays_d = torch.randn(H * W, 3).to(device)              # Random direction for now
    ...
    for i in range(1000):
        ...
        if i % 100 == 0:
            ...
            plt.axis('off')
            plt.savefig(f"render_step_{i}_with_image.png")
            plt.close()
