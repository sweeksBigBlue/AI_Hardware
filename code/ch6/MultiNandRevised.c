...
void print_help(const char *prog_name) {
    ...
    printf("\nOptions:\n");
    printf("  -n  Number of binary inputs (e.g. -n 6)\n");
...
int main(int argc, char *argv[]) {
    ...
    double bias = n_inputs * 1.0; // Start with a bias large enough to oppose input sum
    double lr = (n_inputs >= 6) ? 0.5 : 0.1;
    int epochs = (n_inputs >= 6) ? 50000 : 10000;

    // Initialize weights with small negative values
